...
(async () => {
    ...
    joinid varchar(2048) not null,
    joinmsg varchar(2048) not null,
    leaveid varchar(2048) not null,
    leavemsg varchar(2048) not null,
    PRIMARY KEY (id)
)`).run()
    ...
    bot.db.addxp = async (userid, xp, message) => {
        return new Promise(async (resolve, reject) => {
            ...
            if (userdata.exp >= nxtlvlexp) {
                userdata.level++;
                if (message.channel) message.channel.send(`<:achievement:716859872578371625> ยินดีด้วยคุณ ${message.author} เลื่อนระดับเป็นเลเวล **${userdata.level}** แล้วค่ะ !`)
